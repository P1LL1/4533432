...
export const data = new SlashCommandBuilder()
  .setName("bgen")
  .setDescription("Generate a basic item from stock")
...
export async function execute(
  ...
  if (!isOwner(interaction.user.id) && !isAllowedChannel(interaction.channelId, "BASIC_GEN_CHANNEL_ID")) {
    await interaction.reply({
      content: `❌ This command can only be used in ${allowedChannelMentions("BASIC_GEN_CHANNEL_ID")}.`,
      flags: 64,
    });
    return;
  }
  ...
  if (serviceName) {
    ...
    if (found.tier !== "basic") {
      await interaction.reply({
        content: `❌ **${found.name}** is not a basic service. Try \`/fgen\` or \`/xgen\` instead.`,
        flags: 64,
      });
      return;
    }
    ...
    const basicServices = await getServices("basic");
    if (basicServices.length === 0) {
      await interaction.reply({ content: "❌ No basic services are available.", flags: 64 });
      return;
    }
    service = basicServices[Math.floor(Math.random() * basicServices.length)]!;
  }
  ...
  const dmSent = await sendGenDM(interaction.user, service.name, item, "basic");
  ...
  if (process.env.GEN_LOG_CHANNEL_ID) {
    try {
      ...
      await ch.send({
        embeds: [
          new EmbedBuilder().setColor(0x5865f2).setTitle("📋 Basic Gen")
            .addFields(
