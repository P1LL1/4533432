...
export const data = new SlashCommandBuilder()
  .setName("fgen")
  .setDescription("Generate a free item from stock")
...
export async function execute(
  ...
  if (!isOwner(interaction.user.id) && !isAllowedChannel(interaction.channelId, "FREE_GEN_CHANNEL_ID")) {
    await interaction.reply({
      content: `❌ This command can only be used in ${allowedChannelMentions("FREE_GEN_CHANNEL_ID")}.`,
      flags: 64,
    });
    return;
  }
  ...
  if (serviceName) {
    ...
    if (found.tier !== "free") {
      await interaction.reply({
        content: `❌ **${found.name}** is not a free service. Try \`/bgen\` or \`/xgen\` instead.`,
        flags: 64,
      });
      return;
    }
    ...
    const freeServices = await getServices("free");
    if (freeServices.length === 0) {
      await interaction.reply({ content: "❌ No free services are available.", flags: 64 });
      return;
    }
    service = freeServices[Math.floor(Math.random() * freeServices.length)]!;
  }
  ...
  const dmSent = await sendGenDM(interaction.user, service.name, item, "free");
  ...
  if (process.env.GEN_LOG_CHANNEL_ID) {
    try {
      ...
      await ch.send({
        embeds: [
          new EmbedBuilder().setColor(0x57f287).setTitle("📋 Free Gen")
            .addFields(
